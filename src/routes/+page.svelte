...
<!-- ХЕДЕР: Утонченный, только чистый текст и аккуратные ховеры -->
<header class="fixed top-0 left-0 w-full z-50 bg-white/90 backdrop-blur-md px-6 md:px-12 py-4 flex justify-between items-center border-b border-[#E5E7EB]">
	...
	<!-- Контакты: Чистая типографика без рамок и скруглений -->
	<div class="hidden sm:flex items-center gap-6 text-sm font-medium">
		<a 
			href="mailto:ustinaeskova@gmail.com" 
			class="text-[#6B7280] hover:text-[#EAB308] transition-colors"
		>
			Email
		</a>
		<a 
			...
			class="text-[#6B7280] hover:text-[#F472B6] transition-colors"
		>
			LinkedIn
		</a>
		<a 
			...
			target="_blank" rel="noopener noreferrer"
			class="text-[#2DD4BF] hover:text-[#14B8A6] transition-colors flex items-center gap-1 font-semibold group"
		>
			<span>Telegram</span>
	...
	<!-- Переключение языка -->
	<div class="flex items-center gap-4 md:gap-6 text-sm text-[#6B7280]">
		<button
			onclick={() => lang = 'ru'}
...
<div class="relative max-w-[1200px] mx-auto px-6 md:px-12 min-h-screen pt-28 pb-32 bg-white">
		...
		<div class="space-y-0">
			...
			<section class="relative py-12 md:py-16 border-t border-[#E5E7EB]">
				...
				<button onclick={() => toggleSection('dev')} class="w-full text-left group">
					<h2 class="text-2xl md:text-3xl font-medium tracking-tight text-[#1B1B1B] group-hover:text-[#2DD4BF] transition-colors flex justify-between items-start gap-4">
					...
					<div transition:slide={{ duration: 500, axis: 'y' }} class="pt-12 space-y-14">
						...
						<article class="grid grid-cols-1 md:grid-cols-[110px_1fr] gap-3 md:gap-8">
							...
							<div>
								...
								<div class="mt-4 text-sm text-[#6B7280]">
									<span class="text-[#6B7280] font-medium">{lang === 'ru' ? 'Заказчик:' : 'Client:'}</span> {lang === 'ru' ? 'Медико-психологический中心' : 'Medical-Psychological Center'}
								</div>
								<div class="mt-4 flex flex-wrap gap-2">
